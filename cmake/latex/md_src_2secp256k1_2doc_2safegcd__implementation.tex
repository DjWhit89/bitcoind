...
\label{md_src_2secp256k1_2doc_2safegcd__implementation}\index{The safegcd implementation in libsecp256k1 explained@{The safegcd implementation in libsecp256k1 explained}}
\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md202}%


This document explains the modular inverse and Jacobi symbol implementations in the {\ttfamily src/modinv$\ast$.h} files. It is based on the paper {\texttt{"{}\+Fast constant-\/time gcd computation and modular inversion"{}}} by Daniel J. Bernstein and Bo-\/\+Yin Yang. The references below are for the Date\+: 2019.\+04.\+13 version.

The actual implementation is in C of course, but for demonstration purposes Python3 is used here. Most implementation aspects and optimizations are explained, except those that depend on the specific number representation used in the C code.\doxysection{1. Computing the Greatest Common Divisor (GCD) using divsteps}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md203}
...
It computes the greatest common divisor of an odd integer {\itshape f} and any integer {\itshape g}. Its inner loop keeps rewriting the variables {\itshape f} and {\itshape g} alongside a state variable {\itshape {$\delta$}} that starts at {\itshape 1}, until {\itshape g=0} is reached. At that point, $\ast$$\vert$f$\vert$$\ast$ gives the GCD. Each of the transitions in the loop is called a "{}division step"{} (referred to as divstep in what follows).
...
\item The answer $\ast$$\vert$f$\vert$ = 7$\ast$.
\end{DoxyItemize}
...
\end{DoxyItemize}\doxysection{2. From GCDs to modular inverses}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md204}
We want an algorithm to compute the inverse {\itshape a} of {\itshape x} modulo {\itshape M}, i.\+e. the number a such that {\itshape a\,x=1 mod M}. This inverse only exists if the GCD of {\itshape x} and {\itshape M} is {\itshape 1}, but that is always the case if {\itshape M} is prime and {\itshape 0 $<$ x $<$ M}. In what follows, assume that the modular inverse exists. It turns out this inverse can be computed as a side effect of computing the GCD by keeping track of how the internal variables can be written as linear combinations of the inputs at every step (see the {\texttt{extended Euclidean algorithm}}). Since the GCD is {\itshape 1}, such an algorithm will compute numbers {\itshape a} and {\itshape b} such that a\,x + b\,M = 1$\ast$. Taking that expression {\itshape mod M} gives {\itshape a\,x mod M = 1}, and we see that {\itshape a} is the modular inverse of {\itshape x mod M}.

A similar approach can be used to calculate modular inverses using the divsteps-\/based GCD algorithm shown above, if the modulus {\itshape M} is odd. To do so, compute {\itshape gcd(f=M,g=x)}, while keeping track of extra variables {\itshape d} and {\itshape e}, for which at every step {\itshape d = f/x (mod M)} and {\itshape e = g/x (mod M)}. {\itshape f/x} here means the number which multiplied with {\itshape x} gives {\itshape f mod M}. As {\itshape f} and {\itshape g} are initialized to {\itshape M} and {\itshape x} respectively, {\itshape d} and {\itshape e} just start off being {\itshape 0} ({\itshape M/x mod M = 0/x mod M = 0}) and {\itshape 1} ({\itshape x/x mod M = 1}).
...
Also note that this approach to track {\itshape d} and {\itshape e} throughout the computation to determine the inverse is different from the paper. There (see paragraph 12.\+1 in the paper) a transition matrix for the entire computation is determined (see section 3 below) and the inverse is computed from that. The approach here avoids the need for 2x2 matrix multiplications of various sizes, and appears to be faster at the level of optimization we\textquotesingle{}re able to do in C.\doxysection{3. Batching multiple divsteps}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md205}
Every divstep can be expressed as a matrix multiplication, applying a transition matrix {\itshape (1/2 t)} to both vectors {\itshape [f, g]} and {\itshape [d, e]} (see paragraph 8.\+1 in the paper)\+:
...
Performing multiple divsteps corresponds to a multiplication with the product of all the individual divsteps\textquotesingle{} transition matrices. As each transition matrix consists of integers divided by {\itshape 2}, the product of these matrices will consist of integers divided by $\ast$2\textsuperscript{N}$\ast$ (see also theorem 9.\+2 in the paper). These divisions are expensive when updating {\itshape d} and {\itshape e}, so we delay them\+: we compute the integer coefficients of the combined transition matrix scaled by $\ast$2\textsuperscript{N}$\ast$, and do one division by $\ast$2\textsuperscript{N}$\ast$ as a final step\+:
...
Because the divsteps transformation only ever divides even numbers by two, the result of {\itshape t\,[f,g]} is always even. When {\itshape t} is a composition of {\itshape N} divsteps, it follows that the resulting {\itshape f} and {\itshape g} will be multiple of $\ast$2\textsuperscript{N}$\ast$, and division by $\ast$2\textsuperscript{N}$\ast$ is simply shifting them down\+:
...
The same is not true for {\itshape d} and {\itshape e}, and we need an equivalent of the {\ttfamily div2} function for division by $\ast$2\textsuperscript{N} mod M$\ast$. This is easy if we have precomputed $\ast$1/M mod 2\textsuperscript{N}$\ast$ (which always exists for odd {\itshape M})\+:
...
\begin{DoxyCode}{0}
\DoxyCodeLine{def\ modinv(M,\ Mi,\ x):}
\DoxyCodeLine{\ \ \ \ "{}"{}"{}Compute\ the\ modular\ inverse\ of\ x\ mod\ M,\ given\ Mi=1/M\ mod\ 2\string^N."{}"{}"{}}
\DoxyCodeLine{\ \ \ \ assert\ M\ \&\ 1}
\DoxyCodeLine{\ \ \ \ delta,\ f,\ g,\ d,\ e\ =\ 1,\ M,\ x,\ 0,\ 1}
\DoxyCodeLine{\ \ \ \ while\ g\ !=\ 0:}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \#\ Compute\ the\ delta\ and\ transition\ matrix\ t\ for\ the\ next\ N\ divsteps\ (this\ only\ needs}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \#\ (N+1)-\/bit\ signed\ integer\ arithmetic).}
\DoxyCodeLine{\ \ \ \ \ \ \ \ delta,\ t\ =\ divsteps\_n\_matrix(delta,\ f\ \%\ 2**N,\ g\ \%\ 2**N)}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \#\ Apply\ the\ transition\ matrix\ t\ to\ [f,\ g]:}
\DoxyCodeLine{\ \ \ \ \ \ \ \ f,\ g\ =\ update\_fg(f,\ g,\ t)}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \#\ Apply\ the\ transition\ matrix\ t\ to\ [d,\ e]:}
\DoxyCodeLine{\ \ \ \ \ \ \ \ d,\ e\ =\ update\_de(d,\ e,\ t,\ M,\ Mi)}
\DoxyCodeLine{\ \ \ \ return\ (d\ *\ f)\ \%\ M}

\end{DoxyCode}


This means that in practice we\textquotesingle{}ll always perform a multiple of {\itshape N} divsteps. This is not a problem because once {\itshape g=0}, further divsteps do not affect {\itshape f}, {\itshape g}, {\itshape d}, or {\itshape e} anymore (only {\itshape {$\delta$}} keeps increasing). For variable time code such excess iterations will be mostly optimized away in later sections.\doxysection{4. Avoiding modulus operations}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md206}
...
To deal with the modulus operation in {\ttfamily div2n}, we simply stop requiring {\itshape d} and {\itshape e} to be in range {\itshape [0,M)} all the time. Let\textquotesingle{}s start by inlining {\ttfamily div2n} into {\ttfamily update\+\_\+de}, and dropping the modulus operation at the end\+:
...
Let\textquotesingle{}s look at bounds on the ranges of these numbers. It can be shown that $\ast$$\vert$u$\vert$+$\vert$v$\vert$$\ast$ and $\ast$$\vert$q$\vert$+$\vert$r$\vert$$\ast$ never exceed $\ast$2\textsuperscript{N}$\ast$ (see paragraph 8.\+3 in the paper), and thus a multiplication with {\itshape t} will have outputs whose absolute values are at most $\ast$2\textsuperscript{N}$\ast$ times the maximum absolute input value. In case the inputs {\itshape d} and {\itshape e} are in {\itshape (-\/M,M)}, which is certainly true for the initial values {\itshape d=0} and {\itshape e=1} assuming {\itshape M $>$ 1}, the multiplication results in numbers in range $\ast$(-\/2\textsuperscript{N}M,2\textsuperscript{N}M)$\ast$. Subtracting less than $\ast$2\textsuperscript{N}$\ast$ times {\itshape M} to cancel out {\itshape N} bits brings that up to $\ast$(-\/2\textsuperscript{N+1}M,2\textsuperscript{N}M)$\ast$, and dividing by $\ast$2\textsuperscript{N}$\ast$ at the end takes it to {\itshape (-\/2M,M)}. Another application of {\ttfamily update\+\_\+de} would take that to {\itshape (-\/3M,2M)}, and so forth. This progressive expansion of the variables\textquotesingle{} ranges can be counteracted by incrementing {\itshape d} and {\itshape e} by {\itshape M} whenever they\textquotesingle{}re negative\+:
...
The resulting function takes {\itshape d} and {\itshape e} in range {\itshape (-\/2M,M)} as inputs, and outputs values in the same range. That also means that the {\itshape d} value at the end of {\ttfamily modinv} will be in that range, while we want a result in {\itshape [0,M)}. To do that, we need a normalization function. It\textquotesingle{}s easy to integrate the conditional negation of {\itshape d} (based on the sign of {\itshape f}) into it as well\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{def\ }normalize(sign,\ v,\ M):}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{...}
\DoxyCodeLine{\textcolor{keywordflow}{return}\ normalize(f,\ d,\ M)}

\end{DoxyCode}
\doxysection{5. Constant-\/time operation}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md207}
...
To make the while loop in {\ttfamily modinv} constant time it can be replaced with a constant number of iterations. The paper proves (Theorem 11.\+2) that {\itshape 741} divsteps are sufficient for any {\itshape 256}-\/bit inputs, and {\texttt{safegcd-\/bounds}} shows that the slightly better bound {\itshape 724} is sufficient even. Given that every loop iteration performs {\itshape N} divsteps, it will run a total of {\itshape {$\lceil$}724/N{$\rceil$}} times.

To deal with the branches in {\ttfamily divsteps\+\_\+n\+\_\+matrix} we will replace them with constant-\/time bitwise operations (and hope the C compiler isn\textquotesingle{}t smart enough to turn them back into branches; see {\ttfamily \doxyref{ctime\+\_\+tests.\+c}{p.}{ctime__tests_8c}} for automated tests that this isn\textquotesingle{}t the case). To do so, observe that a divstep can be written instead as (compare to the inner loop of {\ttfamily gcd} in section 1).
...
To convert the above to bitwise operations, we rely on a trick to negate conditionally\+: per the definition of negative numbers in two\textquotesingle{}s complement, ({\itshape -\/v == $\sim$v + 1}) holds for every number {\itshape v}. As {\itshape -\/1} in two\textquotesingle{}s complement is all {\itshape 1} bits, bitflipping can be expressed as xor with {\itshape -\/1}. It follows that {\itshape -\/v == (v $^\wedge$ -\/1) -\/ (-\/1)}. Thus, if we have a variable {\itshape c} that takes on values {\itshape 0} or {\itshape -\/1}, then {\itshape (v $^\wedge$ c) -\/ c} is {\itshape v} if {\itshape c=0} and {\itshape -\/v} if {\itshape c=-\/1}.
...
To use that trick, we need a helper mask variable {\itshape c1} that resolves the condition {\itshape {$\delta$}$>$0} to {\itshape -\/1} (if true) or {\itshape 0} (if false). We compute {\itshape c1} using right shifting, which is equivalent to dividing by the specified power of {\itshape 2} and rounding down (in Python, and also in C under the assumption of a typical two\textquotesingle{}s complement system; see {\ttfamily assumptions.\+h} for tests that this is the case). Right shifting by {\itshape 63} thus maps all numbers in range $\ast$[-\/2\textsuperscript{63},0)$\ast$ to {\itshape -\/1}, and numbers in range $\ast$[0,2\textsuperscript{63})$\ast$ to {\itshape 0}.
...
A variant of divsteps with better worst-\/case performance can be used instead\+: starting {\itshape {$\delta$}} at {\itshape 1/2} instead of {\itshape 1}. This reduces the worst case number of iterations to {\itshape 590} for {\itshape 256}-\/bit inputs (which can be shown using convex hull analysis). In this case, the substitution {\itshape {$\zeta$}=-\/({$\delta$}+1/2)} is used instead to keep the variable integral. Incrementing {\itshape {$\delta$}} by {\itshape 1} still translates to decrementing {\itshape {$\zeta$}} by {\itshape 1}, but negating {\itshape {$\delta$}} now corresponds to going from {\itshape {$\zeta$}} to {\itshape -\/({$\zeta$}+1)}, or $\ast$$\sim${$\zeta$}{\itshape . Doing that conditionally based on $\ast$c3} is simply\+:
...
These bit fiddling tricks can also be used to make the conditional negations and additions in {\ttfamily update\+\_\+de} and {\ttfamily normalize} constant-\/time.\doxysection{6. Variable-\/time optimizations}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md208}
...
It is easy to find what that multiple is\+: we want a number {\itshape w} such that {\itshape g+w\,f} has a few bottom zero bits. If that number of bits is {\itshape L}, we want $\ast$g+w\,f mod 2\textsuperscript{L} = 0$\ast$, or $\ast$w = -\/g/f mod 2\textsuperscript{L}{\itshape . Since $\ast$f} is odd, such a {\itshape w} exists for any {\itshape L}. {\itshape L} cannot be more than {\itshape i} steps (as we\textquotesingle{}d finish the loop before doing more) or more than {\itshape {$\eta$}+1} steps (as we\textquotesingle{}d run {\ttfamily eta, f, g = -\/eta, g, -\/f} at that point), but apart from that, we\textquotesingle{}re only limited by the complexity of computing {\itshape w}.
...
By using a bigger table more bits can be cancelled at once. The table can also be implemented as a formula. Several formulas are known for computing modular inverses modulo powers of two; some can be found in Hacker\textquotesingle{}s Delight second edition by Henry S. Warren, Jr. pages 245-\/247. Here we need the negated modular inverse, which is a simple transformation of those\+:


\begin{DoxyItemize}
\item Instead of a 3-\/bit table\+:
\begin{DoxyItemize}
\item {\itshape -\/f} or {\itshape f $^\wedge$ 6}
\end{DoxyItemize}
\item Instead of a 4-\/bit table\+:
\begin{DoxyItemize}
\item {\itshape 1 -\/ f(f + 1)}
\item {\itshape -\/(f + (((f + 1) \& 4) $<$$<$ 1))}
\end{DoxyItemize}
\item For larger tables the following technique can be used\+: if $\ast$w=-\/1/f mod 2\textsuperscript{L}{\itshape , then $\ast$w(w\,f+2)} is $\ast$-\/1/f mod 2\textsuperscript{2L}$\ast$. This allows extending the previous formulas (or tables). In particular we have this 6-\/bit function (based on the 3-\/bit function above)\+:
\begin{DoxyItemize}
\item $\ast$f(f\textsuperscript{2} -\/ 2)$\ast$
\end{DoxyItemize}
\end{DoxyItemize}

This loop, again extended to also handle {\itshape u}, {\itshape v}, {\itshape q}, and {\itshape r} alongside {\itshape f} and {\itshape g}, placed in {\ttfamily divsteps\+\_\+n\+\_\+matrix}, gives a significantly faster, but non-\/constant time version.\doxysection{7. Final Python version}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md209}
All together we need the following functions\+:


\begin{DoxyItemize}
\item A way to compute the transition matrix in constant time, using the {\ttfamily divsteps\+\_\+n\+\_\+matrix} function from section 2, but with its loop replaced by a variant of the constant-\/time divstep from section 5, extended to handle {\itshape u}, {\itshape v}, {\itshape q}, {\itshape r}\+:
\end{DoxyItemize}
...
\DoxyCodeLine{\ \ \ \ \ \ \ \ f,\ g\ =\ update\_fg(f,\ g,\ t)}
\DoxyCodeLine{\ \ \ \ \ \ \ \ d,\ e\ =\ update\_de(d,\ e,\ t,\ M,\ Mi)}
\DoxyCodeLine{\ \ \ \ \textcolor{keywordflow}{return}\ normalize(f,\ d,\ Mi)}

\end{DoxyCode}
\doxysection{8. From GCDs to Jacobi symbol}\label{md_src_2secp256k1_2doc_2safegcd__implementation_autotoc_md210}
We can also use a similar approach to calculate Jacobi symbol {\itshape (x $\vert$ M)} by keeping track of an extra variable {\itshape j}, for which at every step {\itshape (x $\vert$ M) = j (g $\vert$ f)}. As we update {\itshape f} and {\itshape g}, we make corresponding updates to {\itshape j} using {\texttt{properties of the Jacobi symbol}}\+:
\begin{DoxyItemize}
\item {\itshape ((g/2) $\vert$ f)} is either {\itshape (g $\vert$ f)} or {\itshape -\/(g $\vert$ f)}, depending on the value of {\itshape f mod 8} (negating if it\textquotesingle{}s {\itshape 3} or {\itshape 5}).
\item {\itshape (f $\vert$ g)} is either {\itshape (g $\vert$ f)} or {\itshape -\/(g $\vert$ f)}, depending on {\itshape f mod 4} and {\itshape g mod 4} (negating if both are {\itshape 3}).
\end{DoxyItemize}

These updates depend only on the values of {\itshape f} and {\itshape g} modulo {\itshape 4} or {\itshape 8}, and can thus be applied very quickly, as long as we keep track of a few additional bits of {\itshape f} and {\itshape g}. Overall, this calculation is slightly simpler than the one for the modular inverse because we no longer need to keep track of {\itshape d} and {\itshape e}.

However, one difficulty of this approach is that the Jacobi symbol {\itshape (a $\vert$ n)} is only defined for positive odd integers {\itshape n}, whereas in the original safegcd algorithm, {\itshape f, g} can take negative values. We resolve this by using the following modified steps\+:
...
The algorithm is still correct, since the changed divstep, called a "{}posdivstep"{} (see section 8.\+4 and E.\+5 in the paper) preserves {\itshape gcd(f, g)}. However, there\textquotesingle{}s no proof that the modified algorithm will converge. The justification for posdivsteps is completely empirical\+: in practice, it appears that the vast majority of nonzero inputs converge to $\ast$f=g=gcd(f\textsubscript{0}, g\textsubscript{0})$\ast$ in a number of steps proportional to their logarithm.

Note that\+:
\begin{DoxyItemize}
\item We require inputs to satisfy {\itshape gcd(x, M) = 1}, as otherwise {\itshape f=1} is not reached.
\item We require inputs {\itshape x \&neq; 0}, because applying posdivstep with {\itshape g=0} has no effect.
\item We need to update the termination condition from {\itshape g=0} to {\itshape f=1}.
\end{DoxyItemize}
...
The optimizations in sections 3-\/7 above are described in the context of the original divsteps, but in the C implementation we also adapt most of them (not including "{}avoiding modulus operations"{}, since it\textquotesingle{}s not necessary to track {\itshape d, e}, and "{}constant-\/time operation"{}, since we never calculate Jacobi symbols for secret data) to the posdivsteps version. 